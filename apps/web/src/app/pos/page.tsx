...
export default function PosTerminalPage() {
  ...
  const handleOpenLogoutSummary = async () => {
    ...
    if (res.success && res.data) {
      ...
      setClosingCardTotal(res.data.cardSales !== undefined ? String(res.data.cardSales) : '');
  ...
  // Helper to add declared discrepancy adjustment
  const handleAddClosingAdjustment = () => {
    ...
    const counted = closingCountedCash !== '' ? Number(closingCountedCash) : 0;
    const diff = Math.round((counted - expected) * 100) / 100;
    const currentDeclared = Math.round(
      closingAdjustments.reduce((sum, adj) => sum + Number(adj.amount || 0), 0) * 100
    ) / 100;
    const rem = Math.round((diff - currentDeclared) * 100) / 100;
    const maxAllowed = Math.round(Math.abs(rem) * 100) / 100;
    ...
    // Sign matches the direction of the remaining discrepancy:
    // If rem < 0 (shortage), adjustment is negative; if rem > 0 (overage), adjustment is positive.
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 select-none">
      ...
      {(() => {
        ...
        const cashDiscrepancy = countedCashNum !== null ? Math.round((countedCashNum - sessionExpectedCash) * 100) / 100 : 0;
        const hasCashDiscrepancy = countedCashNum !== null && Math.abs(cashDiscrepancy) >= 0.01;

        const totalDeclaredAdjustments = Math.round(
          closingAdjustments.reduce((sum, adj) => sum + Number(adj.amount || 0), 0) * 100
        ) / 100;

        const remainingDiscrepancy = Math.round((cashDiscrepancy - totalDeclaredAdjustments) * 100) / 100;
        const isDiscrepancyFullyDeclared = Math.abs(remainingDiscrepancy) < 0.01;
        const maxAllowedDiscrepancy = Math.round(Math.abs(remainingDiscrepancy) * 100) / 100;

        const canEndShift =
          countedCashNum !== null &&
          (!hasCashDiscrepancy || isDiscrepancyFullyDeclared) &&
          !isEndingShift;
        ...
        return (
          ...
          >
            <div
              className="space-y-4 text-xs transition-transform"
              style={{ zoom: fontSizeScale === '110%' || fontSizeScale === '1.1' ? 1.05 : 1 }}
            >
              {/* User Details & Shift Duration Header Banner */}
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-xs">
                <div className="flex items-center gap-3">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                      ...
                      <span className="font-mono text-slate-700 dark:text-slate-300 font-bold">{formatShiftStarted(activeShift?.opened_at || sessionSummaryData?.session?.opened_at)}</span>
                    </div>
                  </div>
                </div>
                <div className="sm:text-right shrink-0">
                  <div className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Shift Duration</div>
                  <div className="font-mono text-base font-black text-emerald-600 dark:text-emerald-400 flex items-center sm:justify-end gap-1.5 mt-0.5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xs">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Opening Float</div>
                  <div className="font-mono text-base font-black text-slate-900 dark:text-white mt-1">
                    {formatCurrency(sessionSummaryData?.session?.opening_balance || activeShift?.opening_balance || 150)}
                  </div>
                </div>

                {/* Invoices Count */}
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xs">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Invoices Processed</div>
                  <div className="font-mono text-base font-black text-blue-600 dark:text-sky-400 mt-1">
                ...
                {/* Total Gross Sales */}
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xs">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Total Gross Sales</div>
                  <div className="font-mono text-base font-black text-emerald-600 dark:text-emerald-400 mt-1">
                ...
                {/* Cash Received */}
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xs">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Cash Received</div>
                  <div className="font-mono text-base font-black text-slate-900 dark:text-white mt-1">
                ...
                {/* Mada / Card Sales */}
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xs">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Mada / Card Sales</div>
                  <div className="font-mono text-base font-black text-slate-900 dark:text-white mt-1">
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xs">
                  ...
                  <div className="font-mono text-base font-black text-blue-700 dark:text-sky-400 mt-1">
              ...
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 space-y-3 shadow-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3.5 py-2.5 font-mono text-sm font-bold text-slate-900 dark:text-white focus:border-blue-500 focus:outline-none transition"
                    />
                  </div>
                ...
                <input
                  ...
                  placeholder="Closing handover note or remarks (optional)..."
              ...
              {/* Discrepancy Declaration (Required to exit if difference found) */}
              {hasCashDiscrepancy && (
                <div className="rounded-2xl border border-amber-300 dark:border-amber-900/70 bg-amber-50/60 dark:bg-amber-950/40 p-4 space-y-3">
                  <div className="flex items-center justify-between gap-2 border-b border-amber-200 dark:border-amber-900/50 pb-2.5">
                    <div className="flex items-center gap-2 text-xs font-bold text-amber-900 dark:text-amber-200">
                      <AlertCircle className="h-4 w-4 text-amber-600 dark:text-amber-400 shrink-0" />
                      <span>Discrepancy Declaration</span>
                    </div>
                    <div className="font-mono text-xs font-extrabold">
                      <span className={isDiscrepancyFullyDeclared ? 'text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/80 px-2.5 py-0.5 rounded-lg' : 'text-amber-800 dark:text-amber-300 bg-amber-200/80 dark:bg-amber-900/60 px-2.5 py-0.5 rounded-lg'}>
                        {Number(Math.abs(totalDeclaredAdjustments).toFixed(2))} / {Number(Math.abs(cashDiscrepancy).toFixed(2))} SAR
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 items-center">
                    <div className="relative w-32">
                      <input
                        ...
                        placeholder="Amount"
                        className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-2 font-mono text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                      />
                    </div>
                    ...
                    <input
                      ...
                      className="flex-1 min-w-[180px] rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                    />
                    ...
                    <button
                      ...
                      disabled={
                        ...
                        Number(newClosingAdjAmount) > maxAllowedDiscrepancy ||
                        !newClosingAdjDesc.trim()
                      }
                      className="flex items-center gap-1 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white px-3 py-2 text-xs font-bold transition whitespace-nowrap cursor-pointer disabled:cursor-not-allowed"
                    >
                  ...
                  {/* List of Declared Adjustments */}
                  {closingAdjustments.length > 0 && (
                    <div className="space-y-1.5 pt-1">
                      {closingAdjustments.map((adj, idx) => (
                        <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-2 text-xs shadow-xs">
                          <div className="flex items-center gap-2">
                            <span className={`font-mono font-bold ${adj.amount < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                              {adj.amount > 0 ? '+' : ''}{adj.amount.toFixed(2)} SAR
                            </span>
                            <span className="text-slate-600 dark:text-slate-400">•</span>
                            <span className="text-slate-700 dark:text-slate-300 font-medium">{adj.description}</span>
                          </div>
                          <button
                            type="button"
      ...
      {/* Open Cash Shift Modal with Carry-Forward (Compact & Premium) */}
      <Modal
        ...
        maxWidth="md"
      >
        <form onSubmit={handleOpenShift} className="p-5 sm:p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400">
                  {carryForwardData?.hasLastSession 
                    ? `Carried over from ${carryForwardData.lastClosedBy}`
                    : 'Count physical cash in till drawer to begin'}
                </p>
              </div>
            </div>
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-500/20 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
          ...
          {errorMsg && (
            <div className="flex items-center gap-2 p-3 bg-rose-50 dark:bg-rose-950/40 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-900 rounded-xl text-xs">
              <AlertCircle className="h-4 w-4 shrink-0" />
              <span>{errorMsg}</span>
            </div>
          )}
          ...
          {carryForwardData?.hasLastSession && (
            <div className="rounded-2xl border border-blue-200 dark:border-blue-900/60 bg-blue-50/50 dark:bg-blue-950/30 p-3.5 space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Expected Carry-Forward</span>
                <span className="font-mono text-base font-black text-blue-700 dark:text-blue-300">
                  {formatCurrency(carryForwardData.carryForwardBalance)}
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
                <span>Previous Cashier: <strong className="text-slate-700 dark:text-slate-200">{carryForwardData.lastClosedBy}</strong></span>
                <span>{carryForwardData.lastClosedAt ? formatDateTime(carryForwardData.lastClosedAt) : ''}</span>
              </div>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
              <span>{carryForwardData?.hasLastSession ? 'Actual Cash in Drawer' : 'Starting Cash Float'}</span>
              <span className="text-[11px] text-slate-400 font-mono">SAR</span>
            </div>
            <div className="relative flex items-center rounded-2xl border-2 border-emerald-500/40 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10 bg-slate-50 dark:bg-slate-900/90 shadow-inner px-4 py-2.5 transition">
              <input
                ...
                placeholder="0.00"
                className="w-full bg-transparent font-mono text-2xl sm:text-3xl font-black text-slate-900 dark:text-white placeholder:text-slate-300 dark:placeholder:text-slate-700 focus:outline-none"
              />
              <span className="font-mono text-sm font-black text-emerald-600 dark:text-emerald-400 pl-2 select-none">
                SAR
              </span>
            </div>
          </div>
          ...
          {/* Opening Discrepancy — show when actual differs from carry-forward */}
          {carryForwardData?.hasLastSession && Number(openingFloat) !== carryForwardData.carryForwardBalance && (
            <div className="rounded-2xl border border-amber-200 dark:border-amber-900/60 bg-amber-50/60 dark:bg-amber-950/40 p-3.5 space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-mono text-xs font-black px-2 py-0.5 rounded ${
                  Number(openingFloat) - carryForwardData.carryForwardBalance > 0
                    ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300'
                    : 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300'
                }`}>
                  {Number(openingFloat) - carryForwardData.carryForwardBalance > 0 ? '+' : ''}
                  {(Number(openingFloat) - carryForwardData.carryForwardBalance).toFixed(2)} SAR
                </span>
              </div>
              <p className="text-[11px] text-slate-600 dark:text-slate-400">
                Expected: <span className="font-bold">{formatCurrency(carryForwardData.carryForwardBalance)}</span>, 
                counted: <span className="font-bold">{formatCurrency(Number(openingFloat))}</span>. 
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => setNewOpeningAdjAmount(e.target.value)}
                    placeholder="Diff"
                    className="w-20 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1.5 font-mono text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                  />
                  <input
                    type="text"
                    value={newOpeningAdjDesc}
                    onChange={(e) => setNewOpeningAdjDesc(e.target.value)}
                    placeholder="Reason (e.g. discrepancy)"
                    className="flex-1 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                  />
                  <button
                    type="button"
                    onClick={() => {
                      if (newOpeningAdjAmount && newOpeningAdjDesc.trim()) {
                        setOpeningAdjustments(prev => [...prev, {
                          amount: Number(newOpeningAdjAmount),
                    ...
                    className="rounded-lg bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 text-xs font-bold transition"
                  >
                    <Plus className="h-3.5 w-3.5" />
                  </button>
                </div>
                ...
                {openingAdjustments.length > 0 && (
                  <div className="space-y-1">
                    {openingAdjustments.map((adj, idx) => (
                      <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs">
                        <div className="flex items-center gap-2">
                          <span className={`font-mono font-bold ${adj.amount < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                            {adj.amount > 0 ? '+' : ''}{adj.amount.toFixed(2)} SAR
                          </span>
                          <span className="text-slate-600 dark:text-slate-400 text-[11px]">{adj.description}</span>
                        </div>
                        <button
                          type="button"
          ...
          <Button
            ...
            variant="primary"
            size="lg"
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase tracking-wider text-xs shadow-lg shadow-emerald-600/25 h-12 rounded-xl transition"
